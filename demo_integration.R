...
source("R/zGroups.R")
source("R/zEnvironmental_Utils.R")
source("R/zoomss_model.R")

cat("🌊 ZooMSS Groups Integration Demo\n")
cat("=================================\n\n")

# Demo 1: Model with default groups (automatic)
cat("📋 Demo 1: zoomss_model() with automatic default groups\n")
cat("--------------------------------------------------------\n")
...
cat("✓ Ready to call: zoomss_model(input_params, SaveTimeSteps = FALSE)\n")
cat("  → Model will automatically load default groups\n\n")

# Demo 2: Model with custom groups
cat("📝 Demo 2: zoomss_model() with custom groups\n")
cat("----------------------------------------------\n")

custom_groups <- zGetGroups(source = "default")
...
cat("✓ Ready to call: zoomss_model(input_params, custom_groups, SaveTimeSteps = FALSE)\n\n")
...
cat("  Groups <- zGetGroups(source = 'file', file = '", template_file, "')\n", sep = "")
cat("  results <- zoomss_model(input_params, Groups, SaveTimeSteps = FALSE)\n\n")
...
cat("results <- zoomss_model(input_params, SaveTimeSteps = FALSE)\n\n")

cat("# Advanced - customize on the fly:\n")
cat("Groups <- zGetGroups()  # Get defaults\n")
cat("Groups$W0[1] <- -13           # Modify flagellates\n") 
cat("results <- zoomss_model(input_params, Groups, SaveTimeSteps = FALSE)\n\n")

cat("# Reproducible - use saved files:\n")
cat("Groups <- zGetGroups(source = 'file', file = 'my_groups.csv')\n")
cat("results <- zoomss_model(input_params, Groups, SaveTimeSteps = FALSE)\n\n")
