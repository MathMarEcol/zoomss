...
test_that("plotTimeSeries works with stacked biomass", {
  expect_no_error(plot_obj <- plotTimeSeries(mdl, by = "biomass", stacked = TRUE))
  expect_s3_class(plot_obj, "ggplot")
})

test_that("plotTimeSeries works with proportional biomass", {
  expect_no_error(plot_obj <- plotTimeSeries(mdl, by = "biomass", proportional = TRUE))
  expect_s3_class(plot_obj, "ggplot")
})
...
test_that("plotTimeSeries handles invalid by parameter", {
  ...
  expect_error(plotTimeSeries(mdl, by = "invalid"), "should be one of")
})
...
mdl2 <- mdl
  ...
  mdl2$N <- NULL  # Remove time series data
