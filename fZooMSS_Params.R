...
fZooMSS_Params <- function(Groups, input_params){
  ...
  param <- list(Groups = Groups, # Read in functional group specific parameters from file
                ...
                fish_grps = which(is.na(Groups$prop) == TRUE), # Which rows are fish
                zoo_grps = which(is.na(Groups$prop) == FALSE), # Which rows are zooplankton
                num_zoo = sum(is.na(Groups$prop) == FALSE), # How many zooplankton
                num_fish = sum(is.na(Groups$prop) == TRUE) # How many fish
  )
