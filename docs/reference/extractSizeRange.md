...
Subsets ZooMSS model output to include only specified size range, useful
for focusing analysis on particular size ranges.
...
- var:
  ...
  Character string specifying which variable to extract (e.g., "N", "Z",
  "Growth")
...
This function extracts a subset of size classes from the specified
ZooMSS model variable. Useful for analyzing specific size ranges (e.g.,
microzooplankton, mesozooplankton) or excluding boundary effects from
model analysis. The function converts log10 size values to size class
